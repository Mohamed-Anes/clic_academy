...
function toggleDiv() {
  ...
  if (div.style.display === "none") {
    ...
    // document.getElementById("gotologi").style.display="none"
...
var tableau = document.getElementById("tablelistmot");
...
for (var i = 0; i < donnees.length; i++) {
  ...
  // Parcourir les données de chaque ligne et ajouter des cellules
  for (var j = 0; j < donnees[i].length; j++) {
    // Créer une nouvelle cellule dans la ligne
    var cellule = ligne.insertCell();

    // Définir le contenu de la cellule avec la donnée correspondante
...
//// la colore de button de navegation dnas le dictionnaire 

var searchOptions = document.querySelectorAll(".search-option");
...
// function sendRequestRecherche(query, type_recherche, page, nivs, niveau) {
...
// sendRequestRecherche('ذه', 'l', 0, 'all', 10);
...
document.getElementById('search-button').addEventListener('click', function () {
  ...
  fetch('/search_etudiant/?query=' + my_query)
