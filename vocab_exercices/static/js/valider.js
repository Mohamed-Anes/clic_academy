...
//   var selectedOption = document.querySelector(".search-option.selected");

//   if (!selectedOption) {
//     // Si aucune option n'est sélectionnée, afficher un message d'erreur ou demander à l'utilisateur de sélectionner une option
//     return;
//   }

//   var searchOption = selectedOption.getAttribute("data-value");
//   var searchResults = searchWord(searchTerm, searchOption);
//   displaySearchResults(searchResults);
// });

// function searchWord(term, option) {
//   // Implémentez ici la logique de recherche pour chaque option choisie
//   // et renvoyez les résultats correspondants sous forme de texte ou d'objets
// }
...
//   if (results.length === 0) {
//     searchResultsElement.innerHTML = "لم يتم العثور على أي نتائج.";
//     return;
//   }
...
function ajouterElementsALaListe(mots) {
  ...
  for (var i = 0; i < mots.length; i++) {
    ...
    var motSpan = document.createElement('span');
    motSpan.textContent = mots[i];
    li.appendChild(motSpan);

    ul.appendChild(li);
  }
}

function editMot(element) {
  var li = element.parentNode;
  var motSpan = li.querySelector('span:not(.edit-icon)');
  var mot = motSpan.textContent.trim();
  var nouveauMot = prompt("Modifiez le mot :", mot);

  if (nouveauMot !== null && nouveauMot !== "") {
    motSpan.textContent = nouveauMot;
  }
}
...
function getdefinition() {
  ...
  // ]
  // var listdefiniction = ['كلمة 13', 'كلمة 14', 'كلمة 15'];

  selection = 1
  var listdefiniction = search_result['definitions']
...
function getexemple() {
  ...
  selection = 4
  var list = search_result['examples']
...
function getantonim() {
  ...
  selection = 3
  var list = search_result['antonymes']
  ajouterElementsALaListe(list)
}
...
function getsynonim() {
  ...
  selection = 2
  var list = search_result['synonymes']
  ajouterElementsALaListe(list)
}
...
function fillConjugationTable() {
  ...
  for (var i = 0; i < pronouns.length; i++) {
    ...
    for (var j = 1; j <= 7; j++) {
      ...
      switch (j) {
        case 1:
          cell.textContent = conjugations.pastKnown;
          break;
        case 2:
          cell.textContent = conjugations.presentKnown;
          break;
        case 3:
          cell.textContent = conjugations.command;
          break;
        case 4:
          cell.textContent = conjugations.pastUnknown;
          break;
        case 5:
          cell.textContent = conjugations.presentSubjunctive;
          break;
        case 6:
          cell.textContent = conjugations.presentJussive;
          break;
        case 7:
          cell.textContent = conjugations.presentUnknown;
          break;
        default:
          break;
      }
...
function getconjugaison() {
  ...
  var list = search_result['conjugaison']
  ajouterElementsALaListe(list)
}
...
function editMot(element) {
  ...
  if (nouveauMot !== null && nouveauMot !== "") {
    ...
    switch (selection) {
      case 1:
        search_result.definitions = nouveauMot
        break;
      case 2:
        search_result.synonymes = nouveauMot
        break;
      case 3:
        search_result.antonymes = nouveauMot
        break;
      case 4:
        search_result.examples = nouveauMot
        break;
      case 5:
        search_result.conjugaison = nouveauMot
        break;
      default:
        break;
    }

    li.textContent = nouveauMot;
    addEditIcon(li);
  }
}

function addEditIcon(li) {
  var editIcon = document.createElement("span");
...
// JavaScript code
document.getElementById('search-button').addEventListener('click', function () {
  console.log("recherche de mot : ")
  my_query = document.getElementById('search-input').value

  fetch('/search_expert/?query=' + my_query)
    ...
    .then(data => {
      ...
      for (var i = 0; i < data.lemmes.length; i++) {
        var dictionaryObject = data.lemmes[i];
        // Perform operations on the dictionary object
        console.log('id : ' + dictionaryObject.id + ', lemme : ' + dictionaryObject.lemme);

        // adding lemma to search results
        var newListItem = document.createElement('a')
        newListItem.textContent = dictionaryObject.lemme;
        newListItem.href = "?id=" + dictionaryObject.id;
